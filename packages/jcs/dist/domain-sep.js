...
export function assertKnownSeparator(sep) {
    ...
    if (!known.includes(sep)) {
        throw new Error(`Unknown domain separator: "${sep}". DACS §B.7 declares a closed v0.1 registry; ` +
...
export function assertEmittableSeparator(sep) {
    if (isLegacyReadSeparator(sep)) {
        throw new Error(`Refusing to sign under read-only legacy separator "${sep}". ` +

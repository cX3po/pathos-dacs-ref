...
import { VERIFIER_API_VERSION, indeterminateVerdict, isLoadError, loadBundleSource, verifyDocument } from '../../../src/lib/verify-document.js';
...
export async function main(argv = process.argv.slice(2)) {
    ...
    const loaded = await loadBundleSource({
        file: values['bundle-file'],
        text: values.stdin ? await readStdin() : undefined,
        anchor: values['bundle-anchor'],
        jobId: values.jobId,
        rpc,
    });
    if (isLoadError(loaded)) {
        const verdict = indeterminateVerdict('source', loaded.error, values.jobId ?? 'unknown');
        const result = { apiVersion: VERIFIER_API_VERSION, bundleKind: 'unrecognised', verdict, exitCode: 2 };
        if (!json)
            console.error(`dacs-verifier ${PACKAGE_VERSION}\n  ? source: ${loaded.error}\n\nverdict: INDETERMINATE\n`);
    ...
    const result = await verifyDocument(loaded, {
        ...
        requireSignatures: values['fixture-signatures'] ? false : undefined,
        jobId: values.jobId,
    });
    if (!json) {
...
if (isEntry) {
    main().then((code) => process.exit(code)).catch((err) => {
        console.error(`dacs-verifier: unhandled error (treated as indeterminate): ${err.message}`);
        process.exit(2);
    });
}

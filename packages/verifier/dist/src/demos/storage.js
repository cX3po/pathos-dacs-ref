 ...
 */
export async function resolveByOwnerListing(rpc, expectedOwner, programName, options = {}) {
    ...
    if (!httpRes.ok)
        throw new Error(`getStorageProgramsByOwner HTTP ${httpRes.status} for ${expectedOwner.slice(0, 12)}…`);
    const envelope = (await httpRes.json());
    if (envelope?.result !== 200)
        throw new Error(`getStorageProgramsByOwner RPC returned result=${String(envelope?.result)} for ${expectedOwner.slice(0, 12)}…`);
    const results = envelope.response;
    if (!Array.isArray(results))
        throw new Error(`getStorageProgramsByOwner returned a malformed listing (${typeof results}) for ${expectedOwner.slice(0, 12)}…`);
    ...
    for (const r of results) {
        ...
        if (!r || typeof r !== 'object' || Array.isArray(r))
            throw new Error(`getStorageProgramsByOwner listing entry is malformed for ${expectedOwner.slice(0, 12)}…`);
        const entry = r;
    ...
    if (new Set(matches).size > 1)
        throw new Error(`ambiguous: ${matches.length} programs named "${programName.slice(0, 16)}…" under ${expectedOwner.slice(0, 12)}…`);
    if (matches.length === 0) {
    ...
    if (record && norm(String(record.owner ?? '')) !== norm(expectedOwner))
        return null;
    return record;
}

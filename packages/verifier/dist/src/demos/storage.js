...
export async function anchor(handle, programName, data, options = {}) {
    ...
    if (signedFrom !== undefined && signedFrom !== address)
        throw new Error(`SR-2 anchor of "${programName}": signed transaction is from ${String(signedFrom).slice(0, 12)}, not this wallet`);
    ...
    if (signedHash !== undefined && facts.txHash !== signedHash)
    ...
    if (signedNonce !== undefined && facts.nonce !== signedNonce)
...
export async function anchorFactsFromNode(rpc, storageAddress, options = {}) {
    ...
    if (!program || program.storageAddress !== storageAddress)
        return null;
    const txHash = program.createdByTx;
    if (typeof txHash !== 'string' || !/^[0-9a-f]{64}$/.test(txHash) || typeof program.owner !== 'string' || !program.owner)
        return null;
    const tx = await nodeCallJson(rpc, 'getTxByHash', { hash: txHash }, options.fetchImpl);
    if (!tx || tx.hash !== txHash || !tx.content || tx.content.type !== 'storageProgram' || tx.content.from !== program.owner)
        return null;

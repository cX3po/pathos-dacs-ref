...
export function deriveBundleLogicalAddress(jobId, role) {
...
function publicKeyFor(request, claim) {
 ...
 */
function verifyHashedDomain(domain, signature, hash, publicKey) {
    const hashText = encoder.encode(hex(hash));
    if (verify(domain, signature, hashText, publicKey))
        return true;
...
function bindingSignatureValid(request, binding) {
    if (!isObject(binding) || !isObject(binding.signature))
        return null;
    const signerKey = claimKey(binding.signer);
    if (signerKey === null || claimKey(binding.signature.signer) !== signerKey)
        return null;
    if (binding.signature.algorithm !== 'ed25519')
        return null;
...
function faultedPartyPermitted(bundle, roles) {
    ...
    if (typeof outcome !== 'string' || !OUTCOMES.has(outcome) ||
        ...
        typeof faulted !== 'string')
        return false;
    const partyRoles = new Set(roles.values());
    if (!partyRoles.has(anchoredRole))
        return false;
    if (outcome === 'completed' || outcome === 'failed-substrate')
        return faulted === 'none';
    if (outcome === 'aborted-by-self' || outcome === 'failed-perm')
        return faulted === anchoredRole;
    if (outcome === 'aborted-by-other' || outcome === 'failed-counterparty') {
        return faulted !== 'none' && faulted !== anchoredRole && partyRoles.has(faulted);
    }
    return false;
}
function verifyBundleSignatures(request, bundle, hash, roles) {
    ...
    for (const entry of bundle.signatures) {
        if (!isObject(entry) || entry.algorithm !== 'ed25519')
            return { valid: false, fullySigned: false };
...
export function resolveBundleBinding(request, bindings, fetchBundle) {
    ...
    for (const bucket of buckets.values()) {
        bucket.sort((left, right) => left.binding.bundleContentHash.localeCompare(right.binding.bundleContentHash) ||
    ...
    for (const signer of [...buckets.keys()].sort()) {
        for (const candidate of buckets.get(signer)) {
            ...
            if (!authorized)
                continue; // Authenticated outsider copies are inert.
    ...
    // BB-6 winner selection is full-signature-aware and native-address-ordering-independent.
    ...
    const winner = standing[0].slice().sort((left, right) => Number(right.fullySigned) - Number(left.fullySigned) ||
 ...
 */
function anchorRoleMismatch(anchorKey, copy) {
    if (!ROLES.has(anchorKey))
        return true;
    if (!isObject(copy))
        return true;
    const declared = copy.anchoredByRole;
    if (typeof declared !== 'string' || !ROLES.has(declared))
        return true;
 ...
 */
function impliedFaultSet(bundle, roles) {
    const outcome = bundle.outcome;
    const anchoredRole = bundle.anchoredByRole;
    if (typeof outcome !== 'string' || !OUTCOMES.has(outcome) ||
        typeof anchoredRole !== 'string' || !ROLES.has(anchoredRole))
    ...
    if (isFab) {
        ...
        if (!faultedPartyPermitted(bundle, roles))
            return null;
        return new Set([bundle.faultedParty]);
    }
    // Legacy AttestationBundle: no explicit faultedParty, derive the implied set.
    if (outcome === 'completed' || outcome === 'failed-substrate')
        return new Set(['none']);
    if (outcome === 'aborted-by-self' || outcome === 'failed-perm')
        return new Set([anchoredRole]);
...
function copySignaturesValid(request, bundle, hash, roles) {
    if (!Array.isArray(bundle.signatures) || bundle.signatures.length === 0)
        return false;
...
function phaseSummaryDiverges(a, b) {
    ...
    for (const p of pa) {
        ...
        if (k === null)
            return true; // non-canonicalizable index → fail-closed divergence
    ...
    for (const p of pb) {
        ...
        if (k === null)
            return true; // non-canonicalizable index → fail-closed divergence
        if (seenB.has(k))
            return true; // duplicate index within pb is likewise malformed → fail-closed
        seenB.add(k);
        const other = byIndex.get(k);
        if (other && (other.kind !== p.kind || other.outcome !== p.outcome))
            return true;
    }
    return false;
}
...
function validateFaultCopy(request, copy) {
    if (!isObject(copy))
        return null;
    ...
    if (!copySignaturesValid(request, copy, hash, roles))
        return null;
 ...
 */
export function resolveFaultBundlePointer(request) {
    ...
    const asRequest = { jobId: '', role: 'buyer', publicKeys: request.publicKeys };
    ...
    if (deref === null)
        return invalid('§10.4.2: dereferenced bundle is not a valid §10.4.1 FAB');
 ...
 * -> `fail`.
 */
export function resolveFaultBundlePair(request) {
    if (!request || !isObject(request.copies))
        return invalid('§10.4.3: malformed FAB-pair input');
    const asRequest = { jobId: '', role: 'buyer', publicKeys: request.publicKeys };
    const parsed = [];
    ...
    if (parsed.length === 0)
        return invalid('§10.4.3: no FAB copy present');
    if (parsed.length === 1) {
 ...
 */
export function resolveMixedVersionPair(request) {
    if (!request || !isObject(request.copies))
        return invalid('§10.4.3: malformed mixed-version input');
    const asRequest = { jobId: '', role: 'buyer', publicKeys: request.publicKeys };
    const parsed = [];
    for (const [anchorKey, copy] of Object.entries(request.copies)) {
        // §10.4.2/§248 anchor-address ↔ anchoredByRole integrity (verify-bundle-v1.ts FIX 1).
    ...
    if (parsed.length < 2)
        return invalid('§10.4.3: mixed-version reconciliation needs at least two copies');
    const [first, ...rest] = parsed;
    for (const other of rest) {

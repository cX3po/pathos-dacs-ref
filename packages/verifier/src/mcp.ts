...
async function toolVerifyBundle(args: Json, config: McpConfig): Promise<string> {
  ...
  if (args.rpc !== undefined) throw new Error('rpc is server configuration, not a tool argument');
  try {
    const result = await verifyDocument(bundle, {
      rpc: config.rpc,
      offline: args.offline as boolean | undefined,
      requireSignatures: args.requireSignatures as boolean | undefined,
...
if (isEntry) serve({ rpc: process.env.DACS_VERIFIER_RPC });

...
`.github/workflows/c1-bridge.yml` runs it on `workflow_dispatch` (inputs `sdk_ref`, `standard_ref`) and weekly. `sdk_ref` defaults to `12c5ad3`, the SDK commit whose `DACS_STANDARD_PIN` equals the bridge pin; the SDK's `main` pins a newer Standard commit and fails the bridge's pin check by design. The SDK repository is private, so the workflow needs a read token in the `DACS_SDK_TOKEN` secret and stops with an error before any checkout when it is absent; a run that never executed the bridge is never green.

...
| §6.3.4 Listing logical/native split | CF-4 addressing + DACS-1 publisher CLI + producer listing | `src/dacs1/addressing.ts`, `src/live/listing-wire.ts`, `src/live/producer-listing.ts`, `src/cli/listing-pub.ts` | ✅ the logical address derives from (seller agent DID, listingId, listingVersion) and the producers carry it on the record inside the signed scope (§6.3.4(b)); the Demos write input is the pinned dacs-sdk's program-name form (percent-encoded logical address); native locator is discovery-only |

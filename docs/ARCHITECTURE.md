...
Reads a signed DACS-1 §6.3.4 Listing (`dacsVersion` "1") from disk, verifies it the way a counterparty would (`verifyBundleListing`: the signature over the signature-excluded JCS hash, the seller's presented IdentityBundle under its self-certifying agent DID, presenter = signer), checks the §6.3.4 members, the pipeline's PhaseStep kinds and the on-record CF-4 `logical_address` against (seller claim, listingId, listingVersion) the way the pinned dacs-sdk's `isListing` does, caps the complete signed record at 16 KiB (LR-2), and anchors the complete record via SR-2 under the pinned dacs-sdk's program-name form (the logical address with each `:` percent-encoded), then emits the §6.3.4(c) discovery artifacts. The demo producers (`src/live/organ-gateway.mts`, `src/live/ap2-test-agent.mts`) build, verify and anchor the same shape through `src/live/producer-listing.ts`.

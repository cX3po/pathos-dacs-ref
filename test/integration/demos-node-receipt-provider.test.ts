...
import { claimRefFor } from '../../src/adapters/demos/identity.js';
...
const request = { logicalAddress: 'dacs1:listing:test', contentHash: CONTENT_HASH,
  anchor: { logicalAddress: 'dacs1:listing:test', nativeAddress: NATIVE, transactionRef: { kind: 'demos', value: TX }, writer: claimRefFor(OWNER), nonce: '403' } };
...
test('a confirmed transaction in a confirmed block yields a finalized, fully bound receipt', async () => {
  ...
  assert.equal(receipt.writer, claimRefFor(OWNER));
...
test('a cold request is enriched with the anchor this session wrote; an explicit anchor wins; unknown names pass through', () => {
  const written = { logicalAddress: 'dacs1:listing:test', nativeAddress: NATIVE, transactionRef: { kind: 'demos', value: TX }, writer: claimRefFor(OWNER), nonce: '403' };

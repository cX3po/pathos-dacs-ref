 ...
 */
function controlSignal(verifiedBy: string | undefined, resolver: Record<string, ResolverEntry>): 'control' | 'indeterminate' | 'none' {
  if (!verifiedBy) return 'none';
  const r = resolver[verifiedBy];
  ...
  return 'none';                          // pass-but-existence, or fail/error → no control established
}

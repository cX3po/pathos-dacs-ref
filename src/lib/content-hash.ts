 ...
 */
export function signatureExcludedHash(artifact: unknown): string {
  if (artifact && typeof artifact === 'object' && !Array.isArray(artifact)) {
    const { signature: _signature, signatures: _signatures, ...unsigned } = artifact as Record<string, unknown>;

...
export class StepLog {
  ...
  rollupDecision(): VerifyVerdict['decision'] {
    ...
    if (this.hasFailure()) return 'fail';
    if (this.hasIndeterminate()) return 'indeterminate';
    return 'pass';
  }
}
 ...
 */
export function resolvePrimaryClaimPubkey(bundle: AttestationBundle): { pubkey: Uint8Array; source: string } | null {
...
function verifySignature(bundle: AttestationBundle, log: StepLog): { ok: boolean; signerPubkeyHex: string | null } {
  if (!bundle.signature) {
  ...
  if (!resolved) {
    log.add(
      'verify-signature',
      'indeterminate',
      `bundle.party.primary.scheme="${bundle.party.primary.scheme}" — only "cci" is supported in v0.2 ` +
  ...
  try {
    ...
    log.add('verify-signature', 'fail', `bundle.signature is not valid base64: ${(e as Error).message}`);
    return { ok: false, signerPubkeyHex: null };
  }
  if (sig.length !== 64) {
    log.add('verify-signature', 'fail',
      `malformed signature length: decoded to ${sig.length} bytes; ed25519 requires exactly 64.`);
    return { ok: false, signerPubkeyHex: null };
  }
 ...
 */
async function verifyTwoSidedAnchoring(
  bundle: AttestationBundle,
  rpc: string,
  log: StepLog,
  counterparty?: AttestationBundle,
  fetchImpl: typeof fetchAnchored = fetchAnchored
): Promise<void> {
  const pair = computeAnchorPair(bundle.jobId);
  log.add(
    'two-sided-anchors-derived',
    'pass',
    `jobId=${bundle.jobId} → buyer=${pair.buyer}, seller=${pair.seller}`
  );

  if (counterparty) {
    if (counterparty.jobId !== bundle.jobId) {
      log.add('two-sided-anchoring', 'fail',
        `counterparty bundle jobId=${counterparty.jobId} does not match local bundle jobId=${bundle.jobId}`);
      return;
    }
    if (counterparty.role === bundle.role) {
      log.add('two-sided-anchoring', 'fail',
        `counterparty bundle has same role="${counterparty.role}" — expected the OTHER side per §10.4.2`);
      return;
    }
    ...
    if (!counterpartySigResult.ok) {
      log.add('two-sided-anchoring', 'fail',
        `counterparty bundle signature did NOT verify: ${counterpartySigLog.steps.map(s => s.detail).join('; ')}`);
      return;
    }
    if (!claimRefEqual(bundle.counterparty.primary, counterparty.party.primary) ||
      ...
      log.add('two-sided-anchoring', 'fail',
        `counterparty bundle refers to a different deal — cross-party primary claims (scheme+identifier) don't match (dispute signal per §10.4.2)`);
      return;
    }
    if (bundle.state !== counterparty.state) {
      log.add('two-sided-anchoring', 'fail',
        `state mismatch between party bundles: local="${bundle.state}" counterparty="${counterparty.state}"`);
      return;
    }
    log.add('two-sided-anchoring', 'pass',
      `counterparty bundle supplied inline (role=${counterparty.role}); signature verified, jobId + cross-party claims + state all match`);
    return;
  }
  ...
  if (buyerResult.status === 'error' || sellerResult.status === 'error') {
    ...
    log.add('two-sided-anchoring', 'indeterminate',
      `anchor fetch failed (RPC error, not an absence signal): ` +
  ...
  if (!buyerPresent && !sellerPresent) {
    log.add('two-sided-anchoring', 'indeterminate',
      `neither anchor present at ${rpc} (buyer=${pair.buyer} seller=${pair.seller}); bundle may not have been anchored`);
    return;
  }
  if (buyerPresent && !sellerPresent) {
    log.add('two-sided-anchoring', 'fail',
      `seller anchor absent at ${pair.seller} — per §10.4.3, unilateral ⇒ aborted-by-self for seller`);
    return;
  }
  if (!buyerPresent && sellerPresent) {
    log.add('two-sided-anchoring', 'fail',
      `buyer anchor absent at ${pair.buyer} — per §10.4.3, unilateral ⇒ aborted-by-self for buyer`);
    return;
  }
  ...
  try {
    ...
    const buyerBundle = JSON.parse(typeof buyerData === 'string' ? buyerData : JSON.stringify(buyerData)) as AttestationBundle;
    const sellerBundle = JSON.parse(typeof sellerData === 'string' ? sellerData : JSON.stringify(sellerData)) as AttestationBundle;
    ...
    if (buyerBundle.role !== 'buyer') {
      log.add('two-sided-anchoring', 'fail',
        `bundle at buyer-anchor declares role="${buyerBundle.role}", expected "buyer" per §10.4.2`);
      return;
    }
    if (sellerBundle.role !== 'seller') {
      log.add('two-sided-anchoring', 'fail',
        `bundle at seller-anchor declares role="${sellerBundle.role}", expected "seller" per §10.4.2`);
      return;
    }
    ...
    if (!matchesBuyer && !matchesSeller) {
      log.add('two-sided-anchoring', 'fail',
        `the bundle being verified is not byte-equal to EITHER party-anchored bundle (jobId shared but content differs — likely a third bundle riding along the jobId)`);
      return;
    }
    ...
    if (matchesBuyer && bundle.role !== 'buyer') {
      log.add('two-sided-anchoring', 'fail',
        `local bundle is byte-equal to buyer-anchor bundle but declares role="${bundle.role}"`);
      return;
    }
    if (matchesSeller && bundle.role !== 'seller') {
      log.add('two-sided-anchoring', 'fail',
        `local bundle is byte-equal to seller-anchor bundle but declares role="${bundle.role}"`);
      return;
    }

    if (buyerBundle.jobId !== sellerBundle.jobId || buyerBundle.jobId !== bundle.jobId) {
      log.add('two-sided-anchoring', 'fail',
        `jobId mismatch: buyer-anchor=${buyerBundle.jobId}, seller-anchor=${sellerBundle.jobId}, local=${bundle.jobId}`);
      return;
    }
    ...
    if (!verifySignature(buyerBundle, buyerSigLog).ok) {
      log.add('two-sided-anchoring', 'fail',
        `buyer-anchor bundle signature did NOT verify: ${buyerSigLog.steps.map(s => s.detail).join('; ')}`);
      return;
    }
    const sellerSigLog = new StepLog();
    if (!verifySignature(sellerBundle, sellerSigLog).ok) {
      log.add('two-sided-anchoring', 'fail',
        `seller-anchor bundle signature did NOT verify: ${sellerSigLog.steps.map(s => s.detail).join('; ')}`);
      return;
    }
    ...
    if (!claimRefEqual(buyerBundle.counterparty.primary, sellerBundle.party.primary) ||
      ...
      log.add('two-sided-anchoring', 'fail',
        `cross-party primary-claim mismatch — buyer and seller anchors describe different deals (dispute signal per §10.4.2)`);
      return;
    }
    if (buyerBundle.state !== sellerBundle.state) {
      log.add('two-sided-anchoring', 'fail',
        `state mismatch between party bundles: buyer="${buyerBundle.state}" seller="${sellerBundle.state}"`);
      return;
    }
  } catch (e) {
    log.add('two-sided-anchoring', 'indeterminate',
      `failed to parse one or both anchored bundles for content comparison: ${(e as Error).message}`);
    return;
  }

  log.add('two-sided-anchoring', 'pass',
    `both party anchors present + content cross-checked (buyer ${pair.buyer.slice(0, 16)}…, seller ${pair.seller.slice(0, 16)}…)`);
}

/** Internal: typed result distinguishing not-found from fetch-error. */
type AnchorFetchResult =
  | { status: 'present'; data: unknown }
  | { status: 'absent' }
  | { status: 'error'; error: string };

async function fetchAnchoredWithStatus(
  rpc: string,
  addr: string,
  fetchImpl: typeof fetchAnchored = fetchAnchored
 ...
 */
function claimRefEqual(a: { scheme: string; identifier: string }, b: { scheme: string; identifier: string }): boolean {
 ...
 */
async function walkAttestationRefs(
  bundle: AttestationBundle,
  rpc: string,
  log: StepLog,
  fetchImpl: typeof fetchAnchored = fetchAnchored
  ...
  for (let i = 0; i < refs.length; i++) {
    ...
    if (isStubLocator) {
      ...
      if (ref.anchor.locator !== expectedPrefix) {
        log.add(label, 'fail',
          `stub locator "${ref.anchor.locator}" does not match contentHash prefix "${expectedPrefix}"`);
        failed++;
        continue;
      }
    ...
    try {
      ...
      log.add(label, 'indeterminate',
        `failed to fetch ${ref.anchor.locator} from ${rpc}: ${(e as Error).message}`);
      continue;
    }
    if (!fetched) {
      log.add(label, 'indeterminate',
        `${ref.anchor.locator} not found at ${rpc}`);
      continue;
    }
    ...
    if (isStubType) {
      log.add(label, 'indeterminate',
        `content-hash matches but type carries dahr-stub: prefix — no validator-quorum signature wraps the fetch. ` +
...
export async function verifyBundle(
  ...
  log.add('canonicalize', 'pass',
    `JCS canonical bytes=${canonical.length}, bundleHash=${bundleHashHex.slice(0, 16)}…`);
  ...
  if (!options.skipTwoSidedLookup) {
    ...
    log.add('two-sided-anchoring', 'skipped',
      'skipTwoSidedLookup=true — offline file verification; §10.4.3 unilateral detection not enforced. ' +

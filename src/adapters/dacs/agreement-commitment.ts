...
export interface CommittedAgreement {
  ...
  committedAt: number;
  addresses: { agreement?: { logical: string; native: string }; commitment: { logical: string; native: string } };
}
...
function object(value: unknown): JsonObject {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new AgreementCommitmentError('malformed', 'expected a JSON object');
  return value as JsonObject;
}

function hashWithout(value: JsonObject, field: string): string {
  const copy = { ...value };
  delete copy[field];
...
async function signedValue(signer: AdapterSigner, domain: DomainSeparator, hash: string): Promise<string> {
  return signatureValue(await signer.sign(domain, hash));
}

function listingReference(listing: JsonObject, supplied?: AgreementCommitmentInput['listingRef']) {
  ...
  return actual as { listingId: string; version: number; contentHash: string };
}
...
const NEGOTIATION: Record<string, AgreementDocumentV1['derivedFromPattern']> = {
...
function decimalParts(value: string): { coefficient: bigint; scale: number } {
  if (!/^(?:0|[1-9][0-9]*)(?:\.[0-9]*[1-9])?$/.test(value)) throw new AgreementCommitmentError('price', `non-canonical CD-1 decimal: ${value}`);
...
function roundedPercent(center: string, percent: number, add: boolean): string {
  if (!Number.isSafeInteger(percent) || percent < 0 || (!add && percent >= 100)) throw new AgreementCommitmentError('price', 'invalid negotiable percentage');
  ...
  if (p.scale === 0) return raw;
  const result = `${raw.slice(0, -p.scale)}.${raw.slice(-p.scale)}`.replace(/\.0+$/, '').replace(/(\.[0-9]*[1-9])0+$/, '$1');
  return result;
}

function expectedDeliverable(listing: JsonObject): JsonObject {
  const offering = object(listing.offering);
  const spec = object(offering.deliverable);
  if (typeof spec.deliverableType === 'string' && typeof spec.hash === 'string') return spec;
  if (typeof spec.kind !== 'string') throw new AgreementCommitmentError('deliverable', 'listing deliverable kind is missing');
  const result: JsonObject = { deliverableType: spec.kind, hash: jcsHashHex(spec) };
  if (typeof spec.schemaUrl === 'string') result.schemaUrl = spec.schemaUrl;
  return result;
}

function validateAgreement(listing: JsonObject, agreement: Omit<AgreementDocumentV1, 'signatures'>, committedAt: number, provisional: boolean): void {
  const ref = listingReference(listing);
  if (jcsHashHex(agreement.listingRef) !== jcsHashHex(ref)) throw new AgreementCommitmentError('listing-ref', 'agreement listingRef mismatch');
  const pipeline = listing.pipeline;
  if (!Array.isArray(pipeline) || pipeline.length === 0) throw new AgreementCommitmentError('pipeline', 'listing pipeline must be non-empty');
  ...
  if (commit.length !== 1) throw new AgreementCommitmentError('commit-phase', 'pipeline must select exactly one agreement commitment phase');
  if (commit[0]!.kind === 'commit-payee-bound-agreement') throw new NotSupportedError('commit-payee-bound-agreement is out of scope for this adapter');
  const negotiation = phases.filter((p) => typeof p.kind === 'string' && NEGOTIATION[p.kind]);
  if (negotiation.length !== 1 || NEGOTIATION[negotiation[0]!.kind as string] !== agreement.derivedFromPattern) {
  ...
  let listedPrice: JsonObject;
  if (pricing.kind === 'fixed') listedPrice = object(pricing.price);
  else if (pricing.kind === 'negotiable') listedPrice = object(pricing.bandCenter);
  else if (pricing.kind === 'metered') listedPrice = object(pricing.unitPrice);
  else throw new AgreementCommitmentError('unrecognized-pricing-kind', 'listing pricing kind is not supported');
  if (price.currency !== listedPrice.currency) throw new AgreementCommitmentError('currency', 'agreement and listing currencies differ');
  decimalParts(price.amount);
  if (pricing.kind === 'fixed' || (pricing.kind === 'negotiable' && agreement.derivedFromPattern === 'fixed-price')) {
    if (compareDecimal(price.amount, String(listedPrice.amount)) !== 0) throw new AgreementCommitmentError('price', 'agreement price does not equal listed price');
    ...
    if (!mq || mq.unit !== pricing.unit || !/^(?:0|[1-9][0-9]*)$/.test(mq.quantity)) throw new AgreementCommitmentError('price', 'metered quantity/unit is invalid');
    ...
    if (compareDecimal(price.amount, total) !== 0) throw new AgreementCommitmentError('price', 'metered total does not match listing');
  }
...
function fetchedObject(value: unknown): JsonObject {
  if (typeof value === 'string') return object(JSON.parse(value));
  return object(value);
}

export async function commitAgreement(input: AgreementCommitmentInput, deps: AgreementCommitmentDependencies): Promise<CommittedAgreement> {
  const now = deps.now?.() ?? Date.now();
  if (!input.jobId) throw new AgreementCommitmentError('job-id', 'jobId is required');
  const listingRef = listingReference(input.listing, input.listingRef);
  const roles = new Map(input.parties.map((p) => [p.role, p]));
  if (!roles.get('buyer') || !roles.get('seller')) throw new AgreementCommitmentError('parties', 'agreement requires buyer and seller parties');
  ...
  const pattern = input.derivedFromPattern ?? (() => {
    const pipeline = input.listing.pipeline as JsonObject[];
    return pipeline?.map((p) => NEGOTIATION[String(p.kind)]).find(Boolean);
  })();
  if (!pattern) throw new AgreementCommitmentError('pattern', 'listing has no recognized negotiation phase');
  const unsigned: Omit<AgreementDocumentV1, 'signatures'> = {
  ...
  const agreement: AgreementDocumentV1 = { ...unsigned, signatures: [
  ...
  let agreementAnchor: AgreementAnchorResult | undefined;
  if (input.anchorAgreement !== false) {
    ...
    if (jcsHashHex(fetched) !== jcsHashHex(agreement)) throw new AgreementCommitmentError('cold-read', 'anchored agreement did not resolve byte-equivalently');
  }
  const commitmentUnsigned = {
    finalityCommitmentVersion: '1' as const, jobId: input.jobId, agreementHash, listingRef,
  ...
  const commitment: FinalityCommitmentRecord = { ...commitmentUnsigned, signature: {
...
export async function verifyAgreementCommitmentCold(expected: AgreementCommitmentExpectation, deps: AgreementCommitmentReadDependencies): Promise<{ outcome: 'pass' | 'fail' | 'indeterminate'; detail: string }> {
  ...
  try {
    ...
    catch { return { outcome: 'indeterminate', detail: 'commitment could not be independently resolved' }; }
    if (jcsHashHex(fetched) !== expected.commitmentHash || jcsHashHex(expected.commitment) !== expected.commitmentHash) return { outcome: 'fail', detail: 'commitment content hash mismatch' };
    ...
    catch { return { outcome: 'indeterminate', detail: 'commitment receipt unavailable' }; }
    ...
    validateAgreement(expected.listing, { ...expected.agreement, signatures: undefined } as unknown as Omit<AgreementDocumentV1, 'signatures'>, committedAt, false);

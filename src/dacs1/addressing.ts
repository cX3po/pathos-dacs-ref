...
const REGISTERED_CLAIM_SCHEMES: ReadonlySet<string> = new Set([
  'cci-xm',
  'cci-web2',
  'cci-pqc',
  'cci-ud',
...
export function formatClaimReference(ref: ClaimReferenceParts): string {
  const scheme = ref.scheme.toLowerCase();
  if (!CLAIM_SCHEME.test(scheme)) throw new Error(`invalid ClaimReference scheme: ${ref.scheme}`);
  if (!ref.identifier) throw new Error('ClaimReference identifier must be non-empty');
  const identifier = ref.identifier.normalize('NFC');
  const params = Object.entries(ref.params ?? {}).sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0);
  if (params.length === 0) return `${scheme}:${identifier}`;
  const encoded = params.map(([key, value]) => `${cf4Encode(key)}=${cf4Encode(value)}`).join('&');
  return `${scheme}:${identifier}?${encoded}`;
}

export function assertRegisteredClaimReference(claimReference: string): void {
  const colon = claimReference.indexOf(':');
  if (colon <= 0) throw new Error(`malformed ClaimReference: ${claimReference}`);
...
export function parseListingLogicalAddress(logicalAddress: string): ListingLogicalAddressParts {
  const match = /^dacs1:([^:]+):([^:]+):v([1-9][0-9]*)$/.exec(logicalAddress);
  if (!match) throw new Error(`malformed DACS-1 listing logical address: ${logicalAddress}`);
 ...
 */
export function opaqueListingProgramName(logicalAddress: string): string {
  ...
  if (programName.includes(':')) throw new Error('listing Storage Program name must be colon-free');
  return programName;
}
